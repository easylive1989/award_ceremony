...
test('dungeon waits, opens, unfurls, resets, and cleans up its animations', { timeout: 45000 }, async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.dt-model').getAttribute('data-chest-scale'), '1,1,1');
    assert.equal(await page.locator('.dt-model').getAttribute('data-chest-position'), '0,0,0');
    await seek(5200);
    assert.equal(await page.locator('.dt-model').getAttribute('data-chest-scale'), '1,1,1');
    assert.equal(await page.locator('.dt-model').getAttribute('data-chest-position'), '0,0,0');
    assert(Number(await page.locator('.dt-model').getAttribute('data-lid-angle')) < -1, 'chest remains open after the reveal');
    assert.equal(await page.locator('.dt-model').getAttribute('data-chest-visible'), 'true');
