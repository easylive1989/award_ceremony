...
在皮膚選單選擇「Claude CwC 2026」。沿用黑金 v2 的四秒信封揭曉流程與組別置頂；視覺遵循 CwC Extended 2026 簡報 guide，以 Ivory（`#FAF9F5`）為背景、Slate（`#141413`）為正文、Gray 150（`#F0EEE6`）為形狀、Gray 400（`#B0AEA5`）為線條，並使用 Clay、Mineral、Cactus、Plum、Heather 作為慶祝色。標題使用 Poppins，得獎隊伍使用 Lora；中文分別以 Noto Sans TC 與 Noto Serif TC 補足字形。版面採清楚的左對齊標題、固定邊界、大片留白、圓角形狀與細線 pictogram 語言。
