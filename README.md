...
| `themes/dungeon/` | 地下城石造拱門、火把、SVG 寶箱與羊皮捲軸揭曉動畫 |
...
點擊舞台或按空白鍵後，寶箱先抖動約 1.45 秒，地面金幣與寶石保持固定；完整弧面箱蓋沿後方鉸鏈連續翻開，同時從箱內泛出金光與碎金。羊皮捲軸以小尺寸從箱中抽出，下半部由箱口與前板遮住，半抽出時短暫停留，完全離開箱口後才放大展開，約 5 秒後完整顯示得獎隊伍與中英祝賀文字；捲軸完整展開後，寶箱與地面寶物在約半秒內淡出，名單保持顯示。沿用共用配樂，不另外加入音效。換頁時重置為等待畫面，Esc 返回設定。

場景、箱身與箱蓋均為本機 SVG，無需載入外部圖片，可直接用 `file://` 開啟。文字會隨舞台及長隊名自動縮放；減少動態效果模式仍等待點擊，點擊後直接顯示展開的捲軸。隱藏舞台或頁籤時暫停動畫，換皮時清理監聽器。新增此皮膚不更動預設外觀或既有名單。
